...
public class HarbitoController {
	...
	public ResponseEntity<String> save(@RequestBody Harbito harbito) {
		try {
			this.harbitoService.save(harbito);
			return new ResponseEntity<String>(new ObjectMapper().writeValueAsString(harbito), HttpStatus.CREATED);
		} catch(Exception e) {
			e.printStackTrace();
			return new ResponseEntity<String>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
		}
