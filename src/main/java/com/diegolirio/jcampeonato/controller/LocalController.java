...
public class LocalController {
	...
	public ResponseEntity<String> save(@RequestBody Local local) {
		try {
			this.localService.save(local);
			return new ResponseEntity<String>(new ObjectMapper().writeValueAsString(local), HttpStatus.CREATED);
		} catch(Exception e) {
			e.printStackTrace();
			return new ResponseEntity<String>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
		}
